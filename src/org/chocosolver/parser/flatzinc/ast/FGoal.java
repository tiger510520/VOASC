...
public class FGoal {
	...
	public static ArrayList<IntVar> decVarList = new ArrayList<IntVar>();

	public static IntVar[] geneDecVarFromList() {
		HashSet<String> nameSet = new HashSet<String>();
		ArrayList<IntVar> noDupList = new ArrayList<IntVar>();
		for (int i = 0; i < decVarList.size(); i++) {
			IntVar var = decVarList.get(i);
		...
		decVarList.clear();
		IntVar[] decisionVars = new IntVar[noDupList.size()];
		decisionVars = noDupList.toArray(decisionVars);
		return decisionVars;
	}

	private enum Search {
		seq_search, int_search, bool_search, set_search, warm_start_bool, warm_start_int
	}

	public static void define_goal(Model aModel, List<EAnnotation> annotations, ResolutionPolicy type,
			Expression expr) {
			...
			if (annotations.size() > 1) {
				...
				if (annotation.id.value.equals("seq_search")) {
					...
					strategy = new StrategiesSequencer(aModel.getEnvironment(),
							Arrays.stream(strategies).filter(Objects::nonNull).toArray(AbstractStrategy[]::new));
				} else {
