...
public class VOASC extends AbstractStrategy<IntVar> implements IMonitorDownBranch {
	...
	public VOASC(IntVar[] vars, long seed,String heu) {
		super(vars);
		if (heu.equals("FRBA")) {
		...
		this.valueSelector = new IntDomainMin();
	...
	public Decision<IntVar> getDecision() {
		...
		if (zeroCountGAC == vars.length && zeroCountSAC == vars.length) {
		...
		}else if ((zeroCountGAC == vars.length && zeroCountSAC < vars.length)
				|| (solver.getRestartCount() >= 100 && zeroCountSAC < vars.length)) {
			for (int a = 0; a < vars.length; a++) {
